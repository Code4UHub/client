...
describe('Name validation', () => {
  ...
  describe('General rules', () => {
    ...
    test('Name with forbidden chars', () => {
      ...
      expect(runGeneralRules(id, 'Jesús<script>', '')).toBe(
        GeneralError.noSpecialChar
      );
    });
    describe('Name exceeds max length', () => {
      test('First name is too long', () => {
        expect(runGeneralRules(id, 'Santiago de Alejandro Sebastián', '')).toBe(
          maxCharMessage
        );
        expect(
          runGeneralRules(id, 'Francisco Fernando de las Flores', '')
        ).toBe(maxCharMessage);
      });
      test('Last name is too long', () => {
        expect(
          runGeneralRules(id, 'Dominguez de los Santos Martinez', '')
        ).toBe(maxCharMessage);
        expect(
          runGeneralRules(id, 'Johnson-Martinez Rodriguez de la Luna', '')
        ).toBe(maxCharMessage);
      });
    });
  ...
  describe('Specific name rules', () => {
    describe('Incorrect leading space', () => {
      test('First name leading space', () => {
        expect(runGeneralRules(id, ' Jorge', '')).toBe(
          NameError.leadingSpaceError
        );
        expect(runGeneralRules(id, ' KARLA', '')).toBe(
          NameError.leadingSpaceError
        );
      });
      test('Last name leading space', () => {
        expect(runGeneralRules(id, ' MORALES', '')).toBe(
          NameError.leadingSpaceError
        );
        expect(runGeneralRules(id, ' gonzalez', '')).toBe(
          NameError.leadingSpaceError
        );
      });
    });
    describe('Incorrect ending space', () => {
      test('First name ending space', () => {
        expect(runGeneralRules(id, 'Jesús ', '')).toBe(
          NameError.endingSpaceError
        );
        expect(runGeneralRules(id, 'danna ', '')).toBe(
          NameError.endingSpaceError
        );
      });
      test('Last name ending space', () => {
        expect(runGeneralRules(id, 'Castro ', '')).toBe(
          NameError.endingSpaceError
        );
        expect(runGeneralRules(id, 'RAMÍREZ ', '')).toBe(
          NameError.endingSpaceError
        );
      });
    });
    describe('Incorrect name with no alpha values', () => {
      test('First name with no alpha values', () => {
        expect(runGeneralRules(id, 'Carl0s', '')).toBe(
          NameError.onlyAlphaError
        );
        expect(runGeneralRules(id, 'Fern4nd4', '')).toBe(
          NameError.onlyAlphaError
        );
      });
      test('Last name with no alpha', () => {
        expect(runGeneralRules(id, 'Gonz4lez', '')).toBe(
          NameError.onlyAlphaError
        );
        expect(runGeneralRules(id, '0NT1V3R0S', '')).toBe(
          NameError.onlyAlphaError
        );
      });
    });
