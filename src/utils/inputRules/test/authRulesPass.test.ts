...
function getSizeError(type: 'min'|'max', id: 'password' | 'passwordLogin' | 'passwordConfirmation') {
    if (type === 'max') {
    ...
    describe('Signup password validation', () => {
        ...
        describe('General rules', () => {
            ...
            test('Password with forbidden chars', () => {
                expect(runGeneralRules(id, '\\1==1HackedYou', '')).toBe(GeneralError.noSpecialChar);
            })
            test('Password exceeds max length', () => {
                expect(runGeneralRules(id, 'Password12345678910$&AbcDEF', '')).toBe(getSizeError('max', id));
                expect(runGeneralRules(id, '@Popocatepetl2023Code4U', '')).toBe(getSizeError('max', id));
            });
        ...
        describe('Specific rules signup password', () => {
            test('Password doesnt have uppercase', () => {
                expect(runGeneralRules(id, 'password124?', '')).toBe(PasswordError.noUpperError);
                expect(runGeneralRules(id, 'carlos1234', '')).toBe(PasswordError.noUpperError);
            });
            test('Password doesnt have lowercase', () => {
                expect(runGeneralRules(id, 'CONTRASENA%', '')).toBe(PasswordError.noLowerError);
                expect(runGeneralRules(id, '12345678U', '')).toBe(PasswordError.noLowerError);
            });
            test('Password doesnt have digit', () => {
                expect(runGeneralRules(id, 'DontForgetPass', '')).toBe(PasswordError.noNumberError);
                expect(runGeneralRules(id, 'BestCoderEver', '')).toBe(PasswordError.noNumberError);
            });
            test('Password doesnt have special char', () => {
                expect(runGeneralRules(id, 'MyStrongPass123','')).toBe(PasswordError.noSpecialCharError)
            })
        })
    })
