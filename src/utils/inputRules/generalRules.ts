...
export function generalRules(input: string, id: string, minLength: { [key: string]: number }, maxLength: { [key: string]: number }) {
    if (isEmptyInput(input)) return GeneralError.noEmpty;
    if (input.length < minLength[id]) return `${GeneralError.minCharError} ${minLength[id]} char`;
    if (input.length > maxLength[id]) return `${GeneralError.maxCharError} ${maxLength[id]} char`;
    return correctState
}
